...
// GenerateNanoId generate a nano id with custom alphabet and size but panics on error.
func GenerateNanoId(alphabet string, size int) string {
...
// GenerateNanoUUID generate a nano id with uuid format and panics on error.
func GenerateNanoUUID() string {
	id, err := generate("0123456789abcdefghijklmnopqrstuvwxyz", 32)
	if err != nil {
		panic(err)
	}
	return id
