...
func init() {
	...
	sf = NewSonyflake(st)
	if sf == nil {
		panic("ftasonyflake not created")
	}
...
func TestSonyflakeOnce(t *testing.T) {
	...
	fmt.Println("ftasonyflake id:", id)
...
func TestNilSonyflake(t *testing.T) {
	...
	if NewSonyflake(startInFuture) != nil {
		t.Errorf("ftasonyflake starting in the future")
	}
	...
	if NewSonyflake(noMachineID) != nil {
		t.Errorf("ftasonyflake with no machine id")
	}
	...
	if NewSonyflake(invalidMachineID) != nil {
		t.Errorf("ftasonyflake with invalid machine id")
	}
}
